...
if(debug_mode ) then world.logInfo("come out to the coast, we'll get together, run things in a co-routine. heh heh heh") end
		-- main_threaded();
		if not(co) then
...
if(debug_mode ) then world.logInfo("main_threaded @ cleanupStage " .. self.miab.cleanupStage) end
...
if(debug_mode ) then world.logInfo("destroyBlocks") end
	local dist = world.distance({self.miab.boundingBox[3], self.miab.boundingBox[4]}, {self.miab.boundingBox[1], self.miab.boundingBox[2]})
	local xMax = dist[1]
	local yMax = dist[2]
	local allClear = true
